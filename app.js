...
app.get("/districts/:districtId/details/", async (request, response) => {
  const { districtId } = request.params;
  const getAllDistrictByStateQuery = `SELECT state.state_name  AS stateName
    FROM state
        INNER JOIN district ON state.state_id = district.district_id
    WHERE district.district_id = ${districtId};`;
  const district = await db.all(getAllDistrictByStateQuery);
